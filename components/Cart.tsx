...
export const Cart: React.FC<CartProps> = ({ cartItems, onRemoveItem, onBackToShop }) => {
  const totalPrice = cartItems.reduce((total, item) => total + item.userInput.price, 0);
        ...
        <>
                ...
                <div key={item.id} className="flex items-center justify-between bg-white p-4 rounded-lg shadow-md border border-stone-200">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-semibold text-stone-600">{`₹${item.userInput.price.toFixed(2)}`}</p>
                    </div>
                  </div>
                  <button
            ...
            <div className="mt-8 pt-4 border-t">
                <div className="flex justify-between items-center">
                    <span className="text-lg font-bold text-stone-800">Total Price:</span>
                    <span className="text-xl font-bold text-amber-700">{`₹${totalPrice.toFixed(2)}`}</span>
                </div>
                <button
                    className="mt-6 w-full px-8 py-3 bg-amber-600 text-white font-bold rounded-lg shadow-lg hover:bg-amber-700 focus:outline-none focus:ring-4 focus:ring-amber-300 transition-all"
                >
                    Proceed to Checkout
                </button>
            </div>
